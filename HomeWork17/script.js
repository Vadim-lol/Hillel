...
class User {
	...
	render = function () {
		...
		if (this.courses){
			divUser.appendChild(this.renderCourses(this.courses));
		}
...
class Lector extends User{
	renderCourses = function (courses, divUser) {
		...
		for (let i = 0; i < courses.length; i++){
			...
			for (let key in gradation) {
				if (key >= studentsScore) {
					studentsScoreStr = gradation[key];
					break;
				}
			}
...
for (let i = 0; i < users.length; i++){
	if (users[i].role === 'student') {
		let student = new Student(users[i].name, users[i].age, users[i].img, users[i].role, users[i].courses);
		student.render();
